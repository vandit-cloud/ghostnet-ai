...
seabed that is VERIFIED EMPTY and count how often it reports something. That is
...
def main() -> int:
    ap = argparse.ArgumentParser(description="False-positive rate on verified-empty seabed.")
    ...
    if not background:
        ...
        print("Without verified-empty seabed there is no way to score the")
    ...
    print(f"  split   {args.split}: {len(background)} verified-empty seabed tiles\n")
    ...
    out.write_text(json.dumps({
        ...
        "curve": rows,
        "note": ("False-positive rate on tiles whose labels are empty, i.e. real side-scan "
