...
def count_active_jobs_for_user(db: Session, user_id: uuid.UUID) -> int:
    """How many of this user's surveys currently have a job in an active
    state -- the per-user concurrent-job quota (rate limiting)."""
    return (
        db.query(ProcessingJob)
        .join(Survey, Survey.id == ProcessingJob.survey_id)
        .filter(Survey.created_by_user_id == user_id, ProcessingJob.status.in_(ACTIVE_JOB_STATUSES))
        .count()
    )
...
#: Upper bound on a job-history request. A survey accumulates one job per run,
#: so this is generous in practice; it exists so a caller cannot ask for an
#: unbounded scan of a pathological survey that was re-processed hundreds of
#: times.
MAX_JOB_HISTORY = 50


def list_jobs_for_survey(db: Session, survey_id: uuid.UUID, limit: int = 10) -> list[ProcessingJob]:
    """This survey's runs, newest first.

    `get_latest_job_for_survey` answers "what happened to my run?" for the one
    current run and nothing else, so a re-processed survey had no way to say
    how this run compares with the one before it -- the previous job row exists
    but was unreachable through the API. Ordered by `created_at` descending so
    index 0 is the current run and index 1 is the one it replaced, which is the
    comparison the processing page draws.
    """
    return (
        db.query(ProcessingJob)
        .filter(ProcessingJob.survey_id == survey_id)
        .order_by(ProcessingJob.created_at.desc())
        .limit(min(limit, MAX_JOB_HISTORY))
        .all()
    )
