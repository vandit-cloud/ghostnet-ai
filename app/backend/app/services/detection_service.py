...
def _priority_for(
    ...
    """Triage a detection for the review queue.

    Keyed off `uncertainty`, NOT off raw confidence numbers, and that is the
    whole point of this function.

    The thresholds here used to be 0.85 / 0.80 / 0.65 / 0.50, chosen against
    MockAIAdapter -- which produces confidences from 0.45 to 0.98. The real
    detector does not live on that scale. Its scores are temperature-calibrated
    (T = 2.72), which squashes everything toward 0.5, so measured over 259 real
    detections the distribution was:

        median 0.42     90th pct 0.66     MAXIMUM 0.728

    A calibrated 0.85 requires a raw detector score of 0.9912. So CRITICAL and
    HIGH were unreachable: every real detection fell through to LOW, and the
    review queue silently stopped triaging while looking like it worked.

    `uncertainty` does not have that problem. Member 1 computes it from band
    edges fitted to the model's own distribution, it is a closed three-value
    vocabulary matching the Uncertainty enum, and it stays meaningful across a
    retrain -- which a hardcoded 0.85 does not.

    Note `low` is currently unreachable too (its edge is 0.75, above the
    model's 0.728 ceiling), so in practice this sees `medium` and `high`. The
    branch is kept because a future model will reach it.
    """
    band = (uncertainty or "").lower()

    if detection_class == "ghost_net":
        # The problem statement's actual target: a confident net outranks
        # everything else, and a missed one keeps fishing for years.
        if band == "low":
    ...
    # No uncertainty supplied. Fall back to confidence, on the real scale
    # rather than the mock's -- see the distribution above.
...
def persist_ai_detection(
    ...
    detection = Detection(
        ...
        bbox_w=bbox[2],
        bbox_h=bbox[3],
        mask_polygon=ai_detection.mask,
        review_only=ai_detection.review_only,
        ...
        priority=_priority_for(
            ai_detection.calibrated_confidence,
            ai_detection.class_,
            ai_detection.uncertainty,
        ),
