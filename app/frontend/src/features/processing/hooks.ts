 ...
 *
 * `useLatestJob` covers the current run and nothing else, so a re-processed
 * survey could not say how this run compares with the one it replaced. Index 0
 * is the current run, index 1 the one before it -- which is the whole
 * comparison the processing page draws. */
export function useSurveyJobs(surveyId: string | undefined, limit = 10) {
  return useQuery({
    queryKey: ["survey-jobs", surveyId, limit],
    queryFn: () => apiFetch<ProcessingJob[]>(`/surveys/${surveyId}/jobs?limit=${limit}`),
    enabled: Boolean(surveyId),
  });
}
