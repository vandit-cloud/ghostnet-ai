...
# PART K — COMPLETE IMPLEMENTATION BLUEPRINT
## Page-by-Page + Frontend + Backend + Database + AI/ML + GIS + Real-Time + Security

> This section converts the master plan into an implementation-level contract. It defines what each major screen does, which states it has, how it communicates with the backend, what data it uses, and how the system behaves without unnecessary page reloads.

# K1. Product Roles

Use explicit authorization roles:
...
- view system health
- view audit logs
- manage datasets and operational settings
...
- accept/reject/mark unknown
- add review notes

### VIEWER
Can:
- view authorized dashboards
- view surveys/detections/maps/reports
- cannot alter processing or review decisions

Backend permissions must be enforced for every protected action.
...
Routes may be merged where a drawer/detail workspace provides better UX. Do not create unnecessary separate screens.

---

# K3. Shared Application Shell

Every authenticated screen should share:

```text
Top Bar
├─ Product / current survey
...
├─ Audit
├─ Settings
└─ Users (authorized roles only)

Main workspace
```

The shell must preserve the active survey/context when moving between related screens.

---

# K4. Global Frontend State

Use a clear state architecture.
...
Do not put every piece of state into one global object.

Server state should have clear cache/invalidation rules.

---

# K5. Page Specification — Login

## Purpose
Secure entry to the application.
...
→ redirect to the user's authorized landing page

Do not expose protected dashboard data before authorization succeeds.

---

# K6. Page Specification — Mission Dashboard
...
### Survey map preview
Clicking a detection selects/opens the same detection in the investigation workflow.

## Refresh
Each widget can refresh independently.

## Real-time
New detection/job events update only affected widgets.

---

# K7. Page Specification — Surveys

## Purpose
Manage survey datasets.
...
- Archive
- View report
- Delete only when authorized and policy permits
...
# K8. Page Specification — New Survey / Upload

## Purpose
Create survey and safely ingest data.

## Workflow

```text
Choose/Create Survey
↓
Upload SSS image/log
↓
Upload/associate metadata
...
## Missing metadata
Clearly explain what functionality is affected.

Never silently invent coordinates.

---

# K9. Page Specification — Survey Detail
...
# K10. Page Specification — Live Processing
...
## Show
- stage
- progress if genuinely measurable
- frames completed/total
- errors
- detections
- throughput
- elapsed time
- estimated remaining time only if mathematically supported

## Live results
New detections stream into:
- recent detections
- KPI totals
- map markers where location is available

## Recovery
One bad frame becomes an error record; the survey continues where safe.

---

# K11. Page Specification — Sonar Investigation

This is the primary scientific analysis workspace.
...
## Evidence overlay
Show only computed evidence.

## Synchronization
Clicking an object in the sonar viewer:
→ selects detection
→ centers GIS
→ updates details.
...
# K12. Page Specification — GIS / 3D Survey Map

## Map modes
- 2D
- 3D where supported and data-backed
...
- unknown
- review queue
- natural/rejected if useful
- depth/bathymetry only when real data exists
...
→ select sonar evidence
→ keep filter state.

## Never
Invent geographic geometry or claim synthetic terrain is measured seabed.

---

# K13. Page Specification — Detections
...
# K14. Page Specification — Detection Detail
...
# K15. Page Specification — Review Queue
...
# K16. Page Specification — Reports

## Report types
- full survey report
- filtered detection report
- selected-detection report
- review report
- evaluation report
...
Every report should contain source/model/version information where appropriate.

---

# K17. Page Specification — Analytics
...
Only measured results are shown.

---

# K18. Page Specification — Models
...
Never modify a model used by historical results without versioning.

---

# K19. Page Specification — Datasets
...
- invalid annotations
- hard negatives
- sample viewer where implemented

---

# K20. Page Specification — System Health

Show actual health checks:
...
Do not show a fake "healthy" indicator.

---

# K21. Page Specification — Alerts
...
## Behavior
Clicking an alert must open the relevant entity.
...
# K23. Page Specification — Settings
...
### Processing
- permitted thresholds/configuration where authorized
...
Do not put sensitive server secrets into frontend settings.

---

# K24. Page Specification — User Management
...
# K25. API Contract — Core Endpoint Groups

The exact implementation may use different naming, but the backend must expose equivalent capabilities.
...
GET /api/v1/maps/detections/{detection_id}
```
...
The real application must document these endpoints with OpenAPI.
...
```json
{
  "error": {
    "code": "INVALID_FILE",
    "message": "The uploaded sonar file could not be processed.",
    "request_id": "..."
  }
}
```
...
# K27. Database — Complete Core Entity Model
...
Add PostGIS geometry fields where appropriate.

---

# K29. Database — Survey/File/Frame
...
Do not duplicate the same authoritative value unnecessarily across tables.

---

# K30. Database — Jobs/Events/Reviews
...
## processing_events
Store important processing transitions for debugging/audit.
...
# K31. Database — Security and Audit
...
# K32. Database Indexing

Create indexes based on measured/query requirements.
...
Do not place huge binary sonar files directly into relational rows unless there is a specific justified reason.
...
Deletion must respect authorization and audit requirements.
...
- enforce per-user/per-job limits

If malware scanning is available in the deployment environment, integrate it for untrusted uploads.

---

# K36. Authentication Security

Use:
- strong password hashing
- short-lived access tokens/session credentials
- secure refresh behavior where applicable
...
Do not store authentication secrets in browser local storage when a safer architecture is available.

---

# K37. Authorization Security
...
+
Database access restrictions where appropriate
```

Every protected resource must enforce ownership/survey permissions as appropriate.

---

# K38. API Abuse Protection
...
- expensive unrestricted spatial queries

Use rate limits, quotas, queue limits and resource limits appropriate to deployment size.

---

# K39. Concurrency and Idempotency
...
must handle duplicate requests safely.
...
# K40. Configuration and Environment Management
...
Before deployment, automatically check:
...
# K42. Container / Deployment Security
...
A backup that has never been restored/tested should not be treated as proven recovery.

---

# K44. Disaster Recovery
...
The system should recover without losing already persisted valid detections.

---

# K45. Scientific Traceability / Data Lineage
...
This is one of the most important "real company" characteristics.

---

# K46. Feature Flags

For optional/experimental capabilities, use explicit flags such as:

```text
ENABLE_3D_MAP
ENABLE_SYNTHETIC_GHOST_NET
ENABLE_AUTOENCODER_UNKNOWN
ENABLE_SECOND_STAGE_VERIFIER
ENABLE_EDGE_INFERENCE
```

Do not delete stable architecture merely because an optional feature is disabled.

---

# K47. API Documentation
...
# K48. Accessibility and Internationalization Readiness
...
- unit/timezone abstraction

Keep text and formatting separate from business logic so localization can be added later.

---

# K49. UI Design Principle — Advanced, Not Confusing

The product should look advanced without becoming a flashy technical toy.

Primary information must be immediately understandable:
...
WHY?
Evidence / model / shadow-context

WHAT NEXT?
High Priority / Human Review
```

Use 3D, animation and glass/depth effects only when they improve understanding.

Do not allow decoration to compete with:
- detection
- location
- confidence
- evidence
...
The visual design should feel like a serious marine operations platform.

---

# K50. Real-Time UX Principle

Real-time does not mean constantly reloading the whole application.
...
```text
New detection
→ map marker + detection list + KPI update

Job progress
→ progress area update

New alert
→ alert badge + alert list update

Review decision
→ selected detection + review queue update
```

No unnecessary redirect to Home.

---

# K51. Offline / Degraded Mode UX
...
When offline:
- preserve already loaded information
- keep local inference available where supported
- queue safe local actions only if explicitly designed
- do not falsely claim synchronization

On reconnect:
- reconcile only required stale data
- preserve user state
- report any conflicts

---

# K52. Observability Correlation

Carry a correlation/request ID through:
...
This allows a developer to investigate one detection from end to end.

---

# K53. Resource Governance

Protect system capacity using:
- maximum file size
- maximum concurrent surveys
...
If a resource limit is reached, show a clear message instead of crashing.

---

# K54. Dependency and Supply-Chain Hygiene

Maintain:
- lock files
- dependency versions
- vulnerability checks
- review of critical packages
- removal of unused dependencies

Do not install large libraries just to implement one small UI effect.
...
This complete flow must be testable.

---

# K56. Release Gates

Do not call a release candidate complete until:

```text
[ ] Real SSS data works
[ ] Dataset provenance verified
[ ] Annotation format consistent
[ ] Baseline measured
[ ] Main model measured
[ ] Challenge slices measured
[ ] Calibration measured
[ ] Unknown flow works
[ ] Geotagging validated
[ ] Database persistence works
[ ] GIS synchronization works
[ ] Real-time updates work
[ ] Reconnect works
[ ] Component-level refresh works
[ ] Auth works
[ ] Authorization works
...
[ ] Offline/edge claim tested
[ ] No fabricated metrics
```

---

# K57. Final Architecture Rule

The implementation should preserve clean boundaries:
...
Do not:
- put ML logic directly into React components
...
The frontend can perform presentation-level validation, but the backend remains authoritative.

---

# K58. What Is Truly Required vs Optional

## Required for the core SIH solution
...
Synchronized sonar/map investigation
```

## Optional / only if evidence and time support it
...
Diffusion ghost-net generation
Automated retraining
Complex distributed infrastructure
```

Never let an optional feature block the validated core pipeline.

---

# K59. Final Build Philosophy
...
2. Prove AI baseline
3. Improve AI with measured experiments
4. Prove geotagging
5. Persist real results
6. Build core dashboard
7. Build sonar/GIS investigation
8. Add real-time behavior
9. Add human review
10. Add reports
11. Add security hardening
12. Add observability/recovery
13. Add offline/edge proof
14. Add advanced 3D/UI polish
```

The result should be a **real, coherent product**, not a collection of impressive screens.
