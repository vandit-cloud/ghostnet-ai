...
# 71. CONCRETE PRIVATE / RESTRICTED-DATA ACQUISITION LEADS FOUND BY SEARCH

This section is intentionally practical. These are not “try emailing random companies” suggestions; they are organizations/projects for which the web evidence shows that relevant SSS or ghost-gear data actually exists.

## 71.1 CSR GeoSurveys Ltd. — HIGHEST PRIORITY GHOST-GEAR PRIVATE-DATA LEAD

**Company:** CSR GeoSurveys Ltd., Nova Scotia, Canada

**Why target them:** DFO's AI portal states that its Ghost Gear Detection model used **1,009 side-scan-sonar images supplied by CSR GeoSurveys Ltd.**, containing 1,568 lobster-trap localizations, 1,068 rope localizations and 66 non-gear negatives. citeturn893391search2

CSR states that it owns/operates multiple survey vessels and performs hydrographic/geophysical surveys using advanced survey equipment. citeturn155113search3turn155113search14

**Contact:**
- Email: `info@csrgeosurveys.com`
- Phone: `+1 902-827-4200`
- Website: https://csrgeosurveys.com/contact citeturn155113search0

**Ask for:**
```text
old/anonymized SSS images
ghost-gear survey tiles
rope and trap examples
non-gear negatives
historical training/demo data
small research-only sample
```

**Best request:** ask for 50–200 representative images first, not their entire archive.
...
## 71.2 Commercial Fisheries Research Foundation (CFRF) — NARRAGANSETT BAY DATA

CFRF says its August 2023 directed SSS survey by CSR Geosurvey found **2,200 ghost traps** for removal in Narragansett Bay. The project says it is continuing SSS-guided removal work and will disseminate collected data. citeturn734971search7turn734971search6

**Contact:**
- Email: `admin@cfrfoundation.org`
- Phone: `+1 401-515-4892`
- https://www.cfrfoundation.org/contact-us citeturn155113search2

**Ask for:**
```text
2023 SSS target imagery
pre-removal sonar
post-removal sonar
ghost-trap target list
GPS/coordinates if shareable
depth/substrate/gear-type table
```

**Why it is special:** the same target can potentially have:
```text
sonar BEFORE removal
        +
...
That is excellent ground-truth material.

**Priority:** ★★★★★

---

## 71.3 General Oceans Foundation / OceansWide — MAINE KLEIN SSS

A 2025 Maine survey used **Klein 4900 side-scan sonar**, multiple frequencies/ranges, and surveyed mud, sand and rock substrates. The project page includes ghost-trap/rope examples and wreck + ghost-gear imagery.

Klein Marine Systems also recently described a General Oceans/OceansWide survey covering approximately 1.7 km² and identifying **more than 1,800 traps in one survey area**. citeturn734971search11

**Why target them:**
- modern commercial sonar
- multiple frequency/range settings
- different seabed types
- potentially thousands of new target images
- useful for domain shift vs older datasets

**Data request:**
```text
SSS waterfall imagery
Klein native / XTF export
target spreadsheet
ground-truth recovery list
pre/post removal images
```
...
## 71.4 Gulf of Maine Lobster Foundation — 2021 SSS Survey

GGGI reports that the Gulf of Maine Lobster Foundation, Monterey Bay Diving and a commercial fisher/diver conducted a 2021 Casco Bay side-scan survey using an **EdgeTech 4125** covering approximately 20 square miles. citeturn734971search2turn734971search5

This is a useful historical source because the same region has later/larger surveys, enabling domain comparison.

**Priority:** ★★★★☆

---

## 71.5 Gulf of Maine / Casco Bay — ~1,300 ALDFG targets

GGGI's Signature Gulf of Maine project reports nearly **1,300 ALDFG targets** identified using CSR GeoSurveys SSS. citeturn734971search5
...
## 71.6 Rogue / Vancouver Island — RCRS

Rugged Coast Research Society reports using **side-scan sonar + submersible ROV** in Kyuquot Sound and Hesquiaht Harbour to locate, ground-truth and remove derelict fishing **nets and traps**; about 1.5 tonnes of ghost gear were removed. citeturn155113search11

Their research page says they have surveyed approximately 80% of the west coast of Vancouver Island and have **over 2,000 marine-debris locations mapped**. citeturn155113search8

**Contact:**
- `general@ruggedcoast.ca`
- https://www.ruggedcoastresearchsociety.com/contact citeturn155113search6

**Ask for:**
```text
SSS images
ROV confirmation frames
net/trap target coordinates
marine-debris GIS
pre/post-removal sonar
```
...
## 71.7 Ocean AID — private commercial AI/sonar lead

Ocean AID is a Victoria, BC company working on automatic target recognition from sonar and marine imaging. Ocean Wise describes Ocean AID's real-time ATR system as paired with side-scan sonar to detect and map ghost fishing gear. citeturn734971search9

Current website:
https://oceanaid.ca/

Current contact:
`contact@oceanaid.ca` citeturn155113search5

**Important:** this is not evidence that Ocean AID will release proprietary data. It is a strong **collaboration/data-partnership lead** because they work directly on the same type of SSS ghost-gear AI problem.

Ask for:
```text
research sample
anonymized demo images
hard negatives
failed detections
ghost gear SSS examples
model-evaluation imagery
```
...
# 72. GOVERNMENT ARCHIVES WITH REAL RAW SSS THAT YOU CAN ACTUALLY TARGET

## 72.1 German BSH PINTA — extremely valuable raw SSS archive

The German Federal Maritime and Hydrographic Agency's PINTA data hub exposes individual survey packages with downloadable/lockable data including:
...
For example, site N-03-07 lists:
- processed SSS: 3.8 MB
- **raw SSS: 74.8 GB JSF**
- manual anomaly target lists
- magnetometer data. citeturn893391search0

Site N-09 contains many raw SSS packages:
- 74.8 GB
- 140.7 GB
- 96.0 GB
- 123.5 GB
- 64.7 GB
- 122.8 GB
and corresponding processed SSS/target lists. citeturn893391search1

Other sites such as O-01-03 also expose raw JSF packages; the example shown is **59.1 GB**. citeturn893391search3

### Why this matters

These are not a tiny “dataset.zip”. They are genuine **survey-scale raw sonar archives**.

They can provide:
```text
huge unlabeled SSS pool
hard negatives
wreck targets
debris
anomaly targets
...
Do not download hundreds of GB blindly.

First:
```text
download metadata
download processed target lists
identify target-rich site
download only relevant raw package
```

Then process JSF into image tiles.

**Priority:** ★★★★★

---

# 73. PRIVATE-DATA DISCOVERY VIA PROJECT CHAINS

Use this exact sequence whenever you discover a promising survey:
...
CSR
       ↓
info@csrgeosurveys.com
```

Another:

```text
CFRF Narragansett Bay
       ↓
CSR Geosurvey
       ↓
SSS
...
This is much better than guessing private companies.

---

# 74. RAW-DATA SEARCH TRICK — exact formats

Search the open web for:
...
Also add repository names:

```text
site:zenodo.org XTF sonar
site:figshare.com XTF sonar
site:github.com XTF sonar
site:github.com JSF sonar
site:data.gov "side scan sonar"
site:ncei.noaa.gov JSF sonar
site:pinta.bsh.de JSF
```

---

# 75. AIRCRAFT: private / unusual acquisition routes

## 75.1 Underwater archaeology survey companies

Search:

```text
"aircraft wreck" "side scan sonar" survey company
"aircraft wreck" hydrographic survey
"airplane wreck" underwater archaeology sonar
"WWII aircraft" "side scan sonar" survey
```
...
old survey mosaics
wreck search data
ROV-confirmed aircraft targets
side-scan waterfall files
```

### Why useful

Unlike a curated ML dataset, a wreck survey can include:
```text
aircraft
aircraft fragments
large debris
...
This is perfect for detector + hard-negative development.

---

# 76. NOAA / NCEI raw-data route

NOAA/NCEI is useful for a different “jugaad”:

```text
public cruise metadata
       ↓
trackline identifier
       ↓
archive record
       ↓
data availability
...
NCEI marine-trackline data covers worldwide marine geophysical cruises and includes side-scan sonar holdings. Its SSS archive uses formats including **JSF and HSX**.

For aircraft, search cruise/expedition records connected to:
- missing aircraft
- WWII aircraft
- wreck surveys
...
- seafloor obstruction surveys

---

# 77. “Reverse-engineer the dataset” trick
...
↓
extract data contributors
↓
extract sonar vendors
↓
search each contributor
↓
...
This avoids falsely counting recycled versions as new data.

---

# 78. Image-to-source reverse mining
...
↓
search caption phrase
↓
search image credit
↓
find original organization
↓
request higher-resolution/source data
```

For sonar archives, filenames and figure captions can reveal:
- survey name
- wreck name
- sonar operator
- project year
...
# 79. “Give me your failed detections” request

A very strong private-data request is:

> Please share difficult or false-positive SSS examples rather than only polished positive examples.

Ask for:
```text
rocks mistaken for gear
cables mistaken for nets
shipwreck mistaken for aircraft
shadow-only cases
low-SNR targets
partial targets
buried gear
...
This can be more useful than ordinary positive-only data.

---

# 80. “Give me only derived tiles” request

If a company cannot release raw XTF due to client confidentiality, ask:

```text
Could you share anonymized 512–2048 px SSS image tiles
with no client name, vessel name, exact coordinates,
or commercially sensitive project information?
```

This often has fewer barriers than requesting raw survey files.

---

# 81. “Data donation + attribution” option

Offer:

```text
non-commercial student/hackathon research
source attribution
...
# 82. PRIVATE-DATA REQUEST TABLE — start here

| Priority | Organization / source | Target | Evidence of relevant sonar | Suggested ask |
|---|---|---|---|---|
| ★★★★★ | CSR GeoSurveys | ghost gear | 1,009 SSS images supplied to DFO AI | 50–200 anonymized training images |
| ★★★★★ | CFRF | ghost traps | ~2,200 traps found by SSS | pre/post survey imagery + target list |
| ★★★★★ | RCRS | **nets + traps** | SSS + ROV ground truth | net/trap sonar + ROV |
| ★★★★★ | GGGI Gulf of Maine partners | ghost gear | ~1,300 ALDFG SSS targets | raw/processed SSS |
| ★★★★★ | BSH PINTA | SSS / unknown targets | tens–hundreds GB JSF archives | target-rich survey packages |
| ★★★★★ | DTU Aqua | ghost nets | 600/1600 kHz SSS + ROV | anonymized sonar tiles |
| ★★★★☆ | General Oceans/OceansWide | ghost traps | Klein 4900 SSS | modern multi-frequency examples |
| ★★★★☆ | Ocean AID | ghost gear AI | SSS + ATR | research/demo imagery |
| ★★★★☆ | DFO/CSR portal | ghost gear | 1,009 SSS images | original dataset / annotations |
| ★★★★☆ | NOAA/NCEI | aircraft/wreck/SSS | worldwide marine tracklines | raw JSF/HSX by survey |
| ★★★★☆ | underwater archaeology firms | plane | aircraft wreck SSS | anonymized wreck tiles |
| ★★★★☆ | maritime museums / wreck projects | plane | wreck sonar archives | SSS image collections |

---

# 83. A better target than “private data”: partially private data

A lot of organizations have a middle category:
...
You can use the public metadata to:
```text
identify the survey
locate target
identify sonar
...
Then ask only for:
```text
small anonymized tile set
```

This has a much higher chance of success than requesting entire proprietary archives.
...
circumvent payment/licensing
use stolen credentials
download data from an accidentally exposed private bucket
scrape clearly private client portals
```
...
anonymized subset
license agreement
government/open-data request
```

That gives you a dataset you can actually defend in SIH documentation.

---

# 85. IMMEDIATE ACTION PLAN

### Ghost Net — send requests first

**1. CSR GeoSurveys**
`info@csrgeosurveys.com` citeturn155113search0

**2. CFRF**
`admin@cfrfoundation.org` citeturn155113search2

**3. Rugged Coast**
`general@ruggedcoast.ca` citeturn155113search6

**4. DTU Aqua**
`emp@aqua.dtu.dk` is listed on the Ghost Nets project page. citeturn155113search1
...
```text
NOAA/NCEI
underwater archaeology companies
wreck survey companies
university marine archaeology labs
ROV survey firms
museum maritime archives
```

Ask for **anonymized SSS tiles**, not necessarily raw data.

---

# 86. Best “jugaad” combination for your SIH model
...
```text
        ...
        │           │            │
        ▼           ▼            ▼
   PUBLIC ML    GOVERNMENT    PRIVATE REQUEST
   DATASETS       ARCHIVES       DATA
        │           │            │
        └───────────┼────────────┘
                    ▼
              UNLABELED SSS
...
This is the strongest approach because you are not dependent on the existence of a single perfect `ghost_net + plane` dataset.

---

# 87. New source evidence worth keeping in the project notes

- DFO directly documents the **1,009-image SSS ghost-gear training source from CSR GeoSurveys**. citeturn734971search1
- CFRF directly documents the **2,200-ghost-trap Narragansett Bay SSS survey** and plans to disseminate data. citeturn734971search7
- RCRS documents **SSS + ROV ground-truthing of actual derelict nets and traps** and removal of 1.5 tonnes of gear. citeturn155113search11
- BSH PINTA exposes genuine **JSF raw SSS archives at tens/hundreds of GB per survey**, plus processed target lists. citeturn893391search0turn893391search1
- NOAA provides a real **B-29 side-scan image** with an 11.5 MB original download. citeturn893391search5
- UB88 provides a real towed-SSS aircraft/wreck archive including **F-4 Phantom, P-38, TBM Avenger and Piper Cherokee** examples. citeturn893391search10

---

# 88. Final conclusion

For your project, I would stop searching only for:
...
and start building the acquisition graph:
...
REQUEST
 ↓
ANONYMIZED TILE / RAW FILE
```

The highest-value leads already identified are **CSR GeoSurveys, CFRF, Rugged Coast, DTU Aqua, GGGI projects, General Oceans/OceansWide, Ocean AID, BSH PINTA, and NOAA/NCEI**.

That is the legitimate “jugaad” path to data that ordinary dataset searches do not expose.
